...
{
    sprintf(str, "%s PC=%04d A=%06d X=%06d", self->modo == supervisor ? "SUP " : "usu ", self->PC, self->A, self->X);
}
...
{
    if (self->erro != ERR_OK) {
        sprintf(str, " E=%d(%d) %s", self->erro, self->complemento, err_nome(self->erro));
    } else {
        strcpy(str, "");
    }
}
...
static bool pega_opcode(cpu_t *self, int *popc)
{
    ...
    if (!pega_mem(self, self->PC, popc))
        return false;
    // pode executar se tiver privilégio para isso
    if (self->modo == supervisor || !self->privilegiadas[*popc])
        return true;
...
static bool pega_A1(cpu_t *self, int *pA1) { return pega_mem(self, self->PC + 1, pA1); }
...
bool cpu_interrompe(cpu_t *self, irq_t irq)
{
    ...
    if (self->modo != usuario && self->erro != ERR_CPU_PARADA)
        return false;
